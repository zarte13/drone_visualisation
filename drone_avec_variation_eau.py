...
def animate(frame):
    ...
    acceleration = -2 * (np.cos(2*t) + 1)   # Second derivative of y
    ...
    arrow_scale = 1
    arrow_length = (np.sin(t+np.pi)-1) / 2 * arrow_scale
    ...
    payload_y = - wire_length + 0.05 * np.cos(2*t - 0.5)
    ...
    payload_arrow = Arrow(payload_x, payload_y-trapezoid_height, 0, arrow_length, 
                       width=0.3, 
                       color='green' if arrow_length > 0 else 'green')
    ax.add_patch(payload_arrow)
    
    return drone_body, rotor1, rotor2, wire, payload,drone_connector2,drone_connector1, payload_arrow, inner_payload
...
anim.save('drone_animation_1.gif', writer=writer)